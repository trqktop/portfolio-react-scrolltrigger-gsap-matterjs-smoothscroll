...
export default function Preloader(props) {
    ...
    useLayoutEffect(() => {
        if (blackTheme) {
            preloader.current.classList.add('preloader_black')
        } else {
            preloader.current.classList.remove('preloader_black')
        }
    }, [])
    useEffect(() => {
        setTimeout(() => {
        ...
        (
            <div ref={preloader} className={'preloader'}>
                <svg className="tea" viewBox="0 0 37 48" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <path id="steamL" d="M17 1C17 1 17 4.5 14 6.5C11 8.5 11 12 11 12" strokeWidth="2" strokeLinecap="round"
