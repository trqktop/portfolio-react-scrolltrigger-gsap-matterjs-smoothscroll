import { useEffect, useRef, useState } from "react"
import './WorkElement.css'
export function WorkElement(props) {
    const { data, gsap } = { ...props }
    const [isHover, setHoverState] = useState(false)
    const image = useRef(null)
    function hoverElementHandler(state) {
        setHoverState((prev) => !prev)
    }
    // useEffect(() => {
    //     if (isHover)
    //         gsap.to(image.current, { y: -image.current.offsetHeight + 311, duration: 5 })
    //     else {
    //         gsap.to(image.current, { y: 0, duration: 2 })
    //     }
    // }, [isHover])



    return (
        <li className={'works__item'}
            onMouseEnter={hoverElementHandler} onMouseLeave={hoverElementHandler}
            id={data.projectName}>
            {/* <div className={isHover ? "works__item-overlay works__item-overlay_hover" : 'works__item-overlay'}>
                <ul className="works__item-tech-stack">
                    <li className="works__item-tech works__item-tech_title">TITLE</li>
                    <li className="works__item-tech">hyml</li>
                    <li className="works__item-tech">css</li>
                    <li className="works__item-tech">scss</li>
                </ul>
            </div>
            <div ref={imgContainer} className={isHover ? "works__work-screen-container works__work-screen-container_hover" : "works__work-screen-container"}>
                < img className={'works__work-screen'}
                    src={data.images}
                    alt={data.projectName} />
            </div> */}
            <figure className="works__item-container">
                <div className="works__image-container">
                    < img ref={image}
                        className={isHover ? "works__screen works__screen_hover" : "works__screen"}
                        src={data.images}
                        alt={data.projectName} />
                </div>
                <figcaption className="tech">
                    <div className={isHover ? 'tech__container tech__container_focus' : 'tech__container'}>
                        <h4 className="tech__project-name tech__element">NAME</h4>
                        <ul className="tech__elements">
                            <li className="tech__element">html</li>
                            <li className="tech__element">html</li>
                            <li className="tech__element">html</li>
                        </ul>
                    </div>
                    <div className="tech__project-link">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                            <path
                                d="M22 12C22 16.714 22 19.0711 20.5355 20.5355C19.0711 22 16.714 22 12 22C7.28595 22 4.92893 22 3.46447 20.5355C2 19.0711 2 16.714 2 12C2 7.28595 2 4.92893 3.46447 3.46447C4.92893 2 7.28595 2 12 2"
                                stroke="white" strokeWidth="1.5" strokeLinecap="round" />
                            <path fillRule="evenodd" clipRule="evenodd"
                                d="M12.4697 11.5303C12.1768 11.2374 12.1768 10.7626 12.4697 10.4697L20.1893 2.75H16.6562C16.242 2.75 15.9062 2.41421 15.9062 2C15.9062 1.58579 16.242 1.25 16.6562 1.25H22C22.4142 1.25 22.75 1.58579 22.75 2V7.34375C22.75 7.75796 22.4142 8.09375 22 8.09375C21.5858 8.09375 21.25 7.75796 21.25 7.34375V3.81066L13.5303 11.5303C13.2374 11.8232 12.7626 11.8232 12.4697 11.5303Z"
                                fill="white" />
                        </svg>
                    </div>
                </figcaption>
            </figure>
        </li >
    )
}